 ...
 * License: This code is distributed under GPL License. (refer gpglib/COPYING)
 */
...
/**********************************/
//added to support SELS functions
#include <errno.h>
#include <limits.h>
#include <math.h>
/*****************************/
...
extern char transform_uid[256]; //recipient (user) id defined in gpg.c
/****************************/
struct kidlist_item {
...
/****************************************************************************************************
 * authors:SELS Team 
 * modifications to support SELS server side transformation code
 * ***************************************************************************************************/


//
// Borrowed from parse-packet.c 
// This funciton is originally a static funcion
// We need this funciton to copy encrypted data to output stream
//
static int
...
/*****************************************************************
 * This function returns the 32bit key id of the
 * list public-key (LK). The function assumes that
 * the list key id is available in list.conf file
 * located in the parent directory of the current home
 * directory.
 * That is
 * if home dir = /usr/loacl/mailman/lists/mac14/PSELS/user
 * then list.conf is in /usr/locAl/mailman/lists/mac14/PSELS.
 * This function also assumes that the list key id is available
 * on the first line in list.conf specifically in the format
 * LK_KEY_ID: 678ade54
 * If this changes this function need to be modified accordingly.
 * 
 * created: 11/20/2006; added to this file 8/13/07
 *******************************************************************/ 
static long getListKeyId()
{
	FILE *fp=NULL;
	char* file = NULL;
	char* p =NULL;
	...
	len = strlen(opt.homedir);//get length of homedir path string
	if((file = calloc(len+9, sizeof(char)))==NULL)
	{ //allocate homedir size+9(for list.conf) bytes
		log_error("GPG::calloc failed in getListKeyId");
		return -1;
	}

	file = strncpy(file, opt.homedir, len+1); //copy home dir string into local variable
	p = strrchr(file,'/');//find the last file path delimiter
	p=strncpy(p+1,"list.conf",10); //truncate the path name one directory up and add file name
	//printf("file name is %s\n", file);

	if((fp=fopen(file,&mode))==NULL)
	{
		log_error("GPG::Error opening file %s in function getListKeyId \n", file);
		return -1;
	}

	if((input = calloc(25, sizeof(char)))==NULL)
	{//allocate 25 bytes for input line  
		log_error("GPG::calloc failed in getListKeyId");
                return -1;
	}
		
	if((input = fgets(input,20,fp))==NULL)
	{
		log_error("GPG::no data read from list.conf in func. getListKeyId");
		return -1;
	}
...
//	printf("key is %s\n", hexKey);
	...
	if(sizeof(long)>=4)
	{
		keyid = strtoul(hexKey,&endptr,16);
		if ((errno == ERANGE && (keyid == ULONG_MAX || keyid == 0))|| (errno != 0 && keyid == 0))
		{
			perror("strtol");
			log_error("GPG::getListKeyId: error in key id format in list.conf\n");
	       		return -1; 
		}		

	}else
	{
		keyid = strtoull(hexKey,&endptr,16);
		if ((errno == ERANGE && (keyid == pow(2,(sizeof(long long)*8)) || keyid == 0))|| (errno != 0 && keyid == 0))
		{
			perror("strtol");
			log_error("GPG::getListKeyId: error in key id format in list.conf\n");
	       		return -1; 
        ...
        if (endptr == hexKey) 
	{
		log_error("GPG::getListKeyId:No digits were found in list.conf\n");
		return -1;
	}
...
static int transform(PACKET* pkt)
{
	//char* p = NULL;
	int rc = 0;
	//u32     keyid[2];  
	...
	//
	// retrieve user's public key to get key id part
	// This is needed to replace key id part of the packet
	// Originally the message is encrypted with LK's public key
	// It should be changed to the user's key id for proper decryption
	// at user 
	//
	pk = xmalloc_clear( sizeof *pk );
	pk->req_usage = PUBKEY_USAGE_ENC;
	sprintf( userid, "<%s>", transform_uid );
	if( (rc = get_pubkey_byname( pk, userid, NULL, NULL, 1 )) ) {
		log_error("cannot find user's public key: %s\n", g10_errstr(rc));
		goto leave;
	}
	//
	// retrieve corresponding user's secret key to apply transformation
	//
	sprintf( c_userid, "<C-%s>", transform_uid );
	sk = xmalloc_clear( sizeof *sk );
	sk->req_usage = PUBKEY_USAGE_ENC;
	if( (rc = get_seckey_byname( sk, c_userid, 1 )) ) {
		log_error("cannot find corresponding secret key: %s\n", g10_errstr(rc));
		goto leave;
	}
	//
	// do transformation
	// plain_dek has transformed data
	//
	rc = pubkey_decrypt(sk->pubkey_algo, &plain_dek, enc->data, sk->skey );
	// 
	// public encrypted packet has key id part which is 2 byte long
	// replace LK's key id with user's key id
	//
	enc->keyid[0]=pk->keyid[0];
	enc->keyid[1]=pk->keyid[1];
	// replace the encrypted data part with the transformed one
	enc->data[1] = mpi_copy(plain_dek);

	if( rc )
	{
		log_error("transformation failed: %s\n", g10_errstr(rc) );
		goto leave;
	}
...
/*********************************************************************
 * modified do_proc_packets function that calls transform  * 
 ***********************************************************************/
...
do_proc_packets( CTX c, IOBUF a )
{
    ...
    //out = iobuf_create("testmsgout.txt");
    out = iobuf_create(NULL);
    ...
    if(keyid==-1)
    {
    	log_error("GPG:: do_proc_packets: invalid key id\n");
	rc = -1;
	goto leave;
    }
	...
	// Basically, except PKT_PUBKEY_ENC, just write parsed packets back
	// to output buffer.
	//
	// If PKT_PUBKEY_ENC packet, do transformation
	//
	// In case of encryption data packet, build_packet() does not copy
	// the data itself. we need to copy the data using copy_packet()
	// in addition to invoking build_packet() to complete output
	// operation
	//
	
	if( pkt->pkttype == PKT_PUBKEY_ENC)
	{
		enc = pkt->pkt.pubkey_enc;
		//printf("Packet encrypted with id  %x %x\n",enc->keyid[0],enc->keyid[1]);
		//printf("%s\n",opt.homedir);
		//printf("%x\n",getListKeyId());
		if(enc->keyid[1]==keyid)
		{
		//	printf("Transforming Packet %x %x\n",enc->keyid[0],enc->keyid[1]);
			transform(pkt);
		//	printf("Transfoming complete\n");
		}
	}
	//
	// write packets to output buffer
	//
	...
	//
	// if packet type is encrypted packet, build_packet 
	//
	if( pkt->pkttype == PKT_ENCRYPTED)
	{
	    rc = copy_packet(pkt->pkt.encrypted->buf, out, pkt->pkttype, 
			pkt->pkt.encrypted->len,
			pkt->pkt.encrypted->is_partial );

		if (rc)
		{
			log_error("error copy packet: %s\n", g10_errstr(rc) );
			goto leave;
		}
  ...
  leave:
    iobuf_close(out); //close out buffer
    release_list( c );
    xfree(c->dek);
    free_packet( pkt );
    xfree( pkt );
...
/***********************************************************************************************
 * end of changes to support SELS
 ***********************************************************************************************/
