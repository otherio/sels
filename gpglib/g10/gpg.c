...
#endif
/*************************************************************************
 *code  to support SELS transformation at the server
 *adding a global variable transform_uid to minimize changes
 *--- without a global variable at least three functions need changes
 *changes need
 *authors - SELS Team 
 ************************************************************************/
char transform_uid[256]; //added by bobba 08/13/07 - to support transform
			//function for sels
...
# endif
	  ...
	  case oRecipient: /* store the recipient */
	    ...
	    /******************************************************************
	     * authors: SELS Team
	     * Modification to support SELS transform
	     * copy user id to transform_uid which is a global variable
	     * used by transform function (defined in mainproc.c) when
	     * extracting a corresponding key. Unless using a global variable
	     * 3 functions need modification
	     * transform is done through --decrypt
	     * call step : decrypt_message() -> proc_encryption-packets() ->
	     * do_proc_packets() -> transform()
	     *****************************************************************/
	    strncpy(transform_uid, pargs.r.ret_str, 255);
	    /*****************************************************************/
